...
\end{alignat*}
```
Our PE extends the PE step in \cite{l:21} (which handles the variance of terminal wealth) to account for the variance of accumulated rewards. Algorithm \ref{alg:TD0} summarizes SPERL's PE method.

Our PI step draws directly on \cite{l:21}'s characterization of equilibrium by greedy action selection, summarized in Algorithm \ref{alg:BwUpdate}.

**Remark** The convergence of Algorithm \ref{alg:BwUpdate} to equilibrium is guaranteed by \cite{l:21} when line \ref{VarSPERL-l7}-\ref{VarSPERL-l8} implemented with full sweeps over all possible $x, a, x', a'$. Without full sweeps (in the sample-based case), convergence results are only present for PE by \cite{t:13}. As both results do not extend trivially to our case, the convergence of {\tt MVarSPERL} remains open.
\begin{algorithm}[tb]
...
Our first environment is derived from the portfolio management problem in \cite{t:12}, where an investor, 
...
is then modeled to follow $\tilde{S}_t = S_t - h(\frac{n_t}{\tau})$, with $h:  \mathbb{R} \rightarrow \mathbb{R}^+$ a temporal impact function of the average trading rate. In our work, the market impact functions are specified linearly as $g(v):= \zeta v$ and $h(v):= \rho \, sign(v) + \eta v$, following \cite{a:01}, where $\zeta$ and $\eta$ indicate the strength of permanent and temporary market impact and $\rho$ reflects the fixed cost of a transaction (we set $\rho=.0625$).
the actual cash received for selling each share, i.e., $\tilde{S}_t = S_t - h(\frac{n_t}{\tau})$, where $h$ is a {\color{blue}temporal [how is $\tilde{S}_t$ used in simulation or by the RL agent?]} impact function of the average trading rate. 
